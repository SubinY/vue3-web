...
export const useEditable = ({ state, props }) => {
  const { tableData, editFormModel, editTableFormRef, editFormErrorMsgs, editableCellKeys, editableRowKeys } = state
  ...
  /** 设置表单值 */
  const setEditFormModel = (recordKey, editValue) => {
  ...
  /** 获取要编辑的值 */
  const getEditValue = (recordKey, currentRow, columns) => {
    // 克隆当前行数据作为临时编辑的表单数据，避免直接修改原数据
    const editValue = cloneDeep(currentRow ?? tableData.value.find((n) => n[String(props.rowKey)] === recordKey))
    // 用户设置的默认值优先
   ...
   */
  const startEditable = (recordKey, currentRow) => {
    editableCellKeys.value.clear()
  ...
  /** 进入编辑单元格状态 */
  const startCellEditable = (recordKey, dataIndex, currentRow) => {
    editableRowKeys.value.clear()
  ...
  /** 取消编辑单元格 */
  const cancelCellEditable = (recordKey, dataIndex) => {
  ...
  /** 这行是不是编辑状态 */
  const isEditable = (recordKey) => editableRowKeys.value.has(recordKey)

  /** 获取表单编辑后的值 */
  const getEditFormModel = (recordKey) => Reflect.get(editFormModel.value, recordKey)

  /** 行编辑表单是否校验通过 */
  const validateRow = async (recordKey) => {
   ...
   *  */
  const validateCell = async (recordKey, dataIndex) => {
